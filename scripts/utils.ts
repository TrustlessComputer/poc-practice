...
export const getWallet = (hre: HardhatRuntimeEnvironment, privateKey?: string) => {
  if (!privateKey) {
    ...
    if (!process.env.ZKNET_DEPLOYER_PRIVATE_KEY) throw "⛔️ Wallet private key wasn't found in .env file!";
  }

  const provider = getProvider(hre);
  
  // Initialize zkSync Wallet
  const wallet = new Wallet(privateKey ?? process.env.ZKNET_DEPLOYER_PRIVATE_KEY!, provider);
...
export const deployContract = async (contractArtifactName: string, hre: HardhatRuntimeEnvironment, constructorArguments?: any[], options?: DeployContractOptions) => {
